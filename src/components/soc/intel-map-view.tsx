...
// ─────────────────────────────────────────────────────────────
// Enterprise-Grade Attack Detail Panel (Fixes Bug: closable popup)
// ─────────────────────────────────────────────────────────────
function AttackDetailPanel({
  ...
  useEffect(() => {
    ...
    // Small delay to avoid immediate closing from the click that opened it
  ...
  // Mock MITRE ATT&CK and confidence data
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {/* Close button - FIX: proper X button that works */}
                <motion.button
...
export function IntelMapView() {
  ...
  const handleClosePanel = useCallback(() => {
    setPanelOpen(false);
    // Don't deselect immediately for smoother transition
    setTimeout(() => {
      if (!panelOpen) setSelectedAttack(null);
    }, 300);
  }, [panelOpen]);
  ...
  return (
    <div className="flex h-full gap-4">
      ...
      {/* Attack Detail Panel - Fixed: now a proper side panel that can be closed */}
      <AttackDetailPanel
