...
// --- KPI Card Component ---
function KPICard({ label, value, icon: Icon, color, suffix, sparkline }: {
  label: string;
  value: number;
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-lg border border-[#1e293b] bg-[#1a2332] p-4 transition-all duration-300`}
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-2xl font-bold font-mono" style={{ color }}>
            {displayValue}{suffix || ''}
          </p>
        </div>
        <div className="rounded-md p-1.5" style={{ backgroundColor: `${color}10` }}>
...
function SeverityDonut() {
  ...
  return (
    <div className="rounded-lg border border-[#1e293b] bg-[#1a2332] p-4">
...
function TrendChart() {
  ...
  return (
    <div className="rounded-lg border border-[#1e293b] bg-[#1a2332] p-4">
      <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#94a3b8]">Alert Trend — Last 24h</h3>
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={trendData} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
            <XAxis dataKey="time" tick={{ fill: '#475569', fontSize: 10 }} axisLine={{ stroke: '#1e293b' }} />
            ...
            <Area type="monotone" dataKey="medium" stackId="1" stroke={SEVERITY_COLORS.medium} fill={SEVERITY_COLORS.medium} fillOpacity={0.2} />
            <Area type="monotone" dataKey="low" stackId="1" stroke={SEVERITY_COLORS.low} fill={SEVERITY_COLORS.low} fillOpacity={0.15} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
    ...
    <div className="rounded-lg border border-[#1e293b] bg-[#1a2332] p-4">
      ...
      <div className="space-y-1">
        {recent.map(alert => (
          <div
            key={alert.id}
            className="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-[#0a0e17] transition-colors"
...
function MiniAttackMap() {
  ...
  useEffect(() => {
    ...
    // Draw simple world outline (simplified rectangles for continents)
    ...
    const continents = [
      [0.12, 0.15, 0.18, 0.35], // North America
      [0.17, 0.55, 0.1, 0.25],  // South America
      [0.42, 0.12, 0.15, 0.38], // Europe
      [0.44, 0.5, 0.14, 0.32],  // Africa
      [0.58, 0.12, 0.22, 0.4],  // Asia
      [0.78, 0.55, 0.1, 0.15],  // Australia
    ];
    ...
    recent.forEach(attack => {
      ...
      // Target dot
      ctx.beginPath();
      ctx.arc(tgtX, tgtY, 2, 0, Math.PI * 2);
      ...
      ctx.strokeStyle = `${SEVERITY_COLORS[attack.severity]}40`;
      ctx.lineWidth = 0.8;
      ctx.stroke();
    });
  ...
  return (
    <div className="rounded-lg border border-[#1e293b] bg-[#1a2332] p-4 globe-container relative">
